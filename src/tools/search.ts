    ...
    async ({ query, limit }) => {
      try {
        ...
        for (const obj of result.objects) {
          ...
          lines.push(
            `**Score:** searchScore=${score.final.toFixed(1)} quality=${(score.detail.quality * 100).toFixed(0)}% popularity=${(score.detail.popularity * 100).toFixed(0)}% maintenance=${(score.detail.maintenance * 100).toFixed(0)}%`
          );
